...
class TimelinePageTests(unittest.TestCase):
    def test_page_exposes_analytics_controls_without_rendering_every_row(self) -> None:
        ...
        self.assertIn('data-timeline-sort="source"', page)
        self.assertIn('data-timeline-sort="record"', page)
        self.assertIn('data-timeline-sort="topic"', page)
        self.assertEqual(page.count('class="timeline-table-sort'), 6)
        self.assertIn('<th aria-sort="descending"><button class="timeline-table-sort is-selected" type="button" data-timeline-sort="time"', page)
        ...
        self.assertIn('data-timeline-sort="influence">Rank</button>', page)
