...
<main class="site-main">
  <section class="hero-store">
    ...
    <div class="hero-panel"><div class="panel-label">MARKET PLUGIN · 6 AGENT TOOLS</div><strong>自然语言 → 精确安装</strong><p>网页与 Agent 共用同一份 registry；安装、更新、卸载都由 DSH 请求你批准。</p><div class="hero-tools" aria-label="Dataset versions"><span>ECOSYSTEM DATASET · {esc(dataset_version)}</span><span>MARKET REGISTRY · {esc(market_dataset_version)}</span><span>UPDATED · {esc(date_label(generated_at))}</span></div><div class="hero-tools"><span>search</span><span>details</span><span>plan</span><span>install</span><span>list</span><span>update / remove</span></div></div>
  </section>
  ...
  <section class="market-band" id="market-plugin" aria-labelledby="market-heading"><div class="market-copy"><p class="kicker">THE STORE + ITS AGENT ENTRY</p><h2 id="market-heading">这是 DeepSeek Harness 的 Plugin Store：我们收录插件，Market Plugin 让 Agent 帮你找、帮你装。</h2><p>人在网页里浏览和核对插件；Agent 通过同一份 Registry 按你的自然语言需求搜索。每个可安装条目都保留稳定 ID、精确 spec、版本和来源归因，安装、更新或卸载前都必须由你批准。</p><div class="market-role-map" aria-label="Store and Market Plugin roles"><article><span>THE STORE</span><strong>把插件放到同一处</strong><p>持续聚合公开 Registry Listing，按精确 spec 去重，并保留来源、版本与日期。</p></article><article><span>MARKET PLUGIN</span><strong>让 Agent 使用这家 Store</strong><p>安装一次后，Agent 获得 search、details、plan、install 和 manage 六个工具。</p></article></div><div class="market-outcomes"><span>{stats['market']:,} 个可安装插件</span><span>网页 / Agent 共用 Registry</span><span>变更操作必须批准</span></div><div class="market-links"><a class="button button-primary" href="market.html">进入插件商店</a><a class="button button-quiet" href="register.html">提交一个插件</a><a class="market-text-link" href="https://github.com/Shiyao-Huang/awesome-deepseek-harness-plugin/tree/main/plugin">Market Plugin 能力 ↗</a><a class="market-text-link" href="data/market-registry.json">Registry JSON ↗</a><a class="market-text-link" href="data/market-registry.schema.json">Schema ↗</a></div></div><div class="market-action-stack"><div class="market-install"><span>STEP 1 · INSTALL MARKET PLUGIN ONCE</span><code>dsh plugin --profile web add github:Shiyao-Huang/awesome-deepseek-harness-plugin#path:/plugin</code><button class="copy-install" type="button" data-install="dsh plugin --profile web add github:Shiyao-Huang/awesome-deepseek-harness-plugin#path:/plugin" aria-label="Copy market plugin install command" aria-live="polite">Copy</button><div class="market-install-footer"><small>安装后重启 DSH；这一条命令只安装 Store 入口。</small><a href="http://127.0.0.1:3080/">Open local DSH ↗</a></div></div><div class="market-agent-ask"><span>STEP 2 · ASK IN NATURAL LANGUAGE</span><p>“找一个能搜索公开网页的插件，先展示来源和精确安装 spec，为 web profile 生成计划；等我批准后再安装。”</p><button class="copy-install" type="button" data-install="请在 deeplugin.store 中找一个能搜索公开网页的插件，先展示来源和精确安装 spec，为 web profile 生成安装计划；等我明确批准后再安装。" aria-label="Copy example Agent request" aria-live="polite">Copy request</button><small>Agent 会依次检索、读取详情并生成计划；批准后才修改 profile。</small></div></div></section>
